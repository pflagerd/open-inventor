...
#define _ATOM_(widget,name) (XmInternAtom(XtDisplay(widget),(char*)name,False))
...
//	Constructor
//
SoXtClipboard::SoXtClipboard(Widget w,  // widget for copy/paste data transfers
...
// Use: protected
//
void SoXtClipboard::copy(SoByteStream *byteStream, Time t)
//
////////////////////////////////////////////////////////////////////////
		{
	eventTime = t;
	...
	if (copyBuffer != NULL) {
		...
		XtOwnSelection(widget, clipboardAtom, eventTime,
		...
		if (XGetSelectionOwner(XtDisplay(widget), clipboardAtom)
				== XtWindow(widget)) {
...
// Use: protected
//
void SoXtClipboard::copy(Atom dataType, void *data, uint32_t numBytes, Time t)
//
////////////////////////////////////////////////////////////////////////
		{
	eventTime = t;
...
// Use: public
//
void SoXtClipboard::copy(SoPathList *pathList, Time t)
//
////////////////////////////////////////////////////////////////////////
		{
...
// Use: public
//
void SoXtClipboard::copy(SoPath *path, Time t)
//
////////////////////////////////////////////////////////////////////////
		{
	SoByteStream *bs = new SoByteStream;
...
// Use: public
//
void SoXtClipboard::copy(SoNode *node, Time t)
//
////////////////////////////////////////////////////////////////////////
		{
	SoByteStream *bs = new SoByteStream;
...
// Use: public
//
void SoXtClipboard::paste(Time t, SoXtClipboardPasteCB *pasteDoneFunc,
		void *pasteUserData)
//
////////////////////////////////////////////////////////////////////////
		{
...
//
void SoXtClipboard::getExportTargets(char **value, uint32_t *length)
//
////////////////////////////////////////////////////////////////////////
		{
...
// Use: static, private
//
Boolean SoXtClipboard::exportSelection(Widget widget, Atom *selectionAtom,
		Atom *target, Atom *returnTarget, char **value, uint32_t *length,
		int *format)
//
////////////////////////////////////////////////////////////////////////
		{
	...
	if (selectionAtom == NULL) {
		SoDebugError::post((char*)"SoXtClipboard::exportSelection",
				(char*)"- selectionAtom is NULL");
		return FALSE;
	}
	if (target == NULL) {
		SoDebugError::post((char*)"SoXtClipboard::exportSelection",
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::exportSelection",
				(char*)"- cannot find selection owner");
#endif
		return FALSE;
	}
	clipboard = (SoXtClipboard *) data;
	...
	if (clipboard->copyBuffer == NULL)
		return False;
	if (clipboard->copyBuffer->getData() == NULL
			|| clipboard->copyBuffer->getNumBytes() <= 0)
		return False;
	...
	// Just serve it up.
		...
		if (clipboard->copyDataType == *target) {
			// Copy the data buffer since the requestor will own the data
			char *exportData = (char *) malloc(
					(size_t) clipboard->copyBuffer->getNumBytes());
			if (exportData != NULL) {
				memcpy(exportData, clipboard->copyBuffer->getData(),
		...
		// the requested type.
		if (convertData(widget, clipboard->copyBuffer->getData(),
				clipboard->copyBuffer->getNumBytes(), *target, value, length)) {
			*format = 8;
			*returnTarget = *target;
			ok = True;
		}
...
//  static,public
//
SbBool SoXtClipboard::convertData(Widget widget, void *srcData,
		uint32_t srcNumBytes, Atom desiredType, char **returnData,
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::convertData",
				(char*)"desired conversion type not supported");
#endif
		return FALSE;
	}
...
#ifdef DEBUG
			SoDebugError::post((char*)"SoXtClipboard::convertData",
					(char*)"could not allocate %d bytes for return buffer", srcNumBytes);
#endif
			return False;
		}
	}
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::convertData",
				(char*)"command <%s> failed", conversionCmd);
#endif
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::convertData",
				(char*)"could not open output file %s for reading", outfile);
#endif
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::convertData",
				(char*)"could not malloc %d bytes", len);
#endif
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::convertData",
				(char*)"could not read %d bytes from output file %s", len, outfile);
#endif
...
//  static protected
//
SbBool SoXtClipboard::writeToFile(SbString &tmpfile, void *srcData,
...
////////////////////////////////////////////////////////////////////////
	...
	if (fd <= 0) {
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::writeToFile",
				(char*)"could not open tmp input file %s", filename);
#endif
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::writeToFile",
				(char*)"could not write %d bytes to tmp input file %s",
...
// Use: static, private
//
void SoXtClipboard::importSelectionTargets(Widget widget,
		SoXtClipboard *clipboard, Atom *selectionAtom, Atom *target,
		char *value, uint32_t *length, int *format)
//
////////////////////////////////////////////////////////////////////////
		{
#ifdef DEBUG
	if (clipboard == NULL) {
		SoDebugError::post((char*)"SoXtClipboard::importSelectionTargets",
				(char*)"- clipboard is NULL");
		return;
	}
	if (target == NULL) {
		SoDebugError::post((char*)"SoXtClipboard::importSelectionTargets",
	...
	if (*target == _ATOM_(widget, (char* )"TARGETS") || *target == XA_ATOM) {
		if (*format == 32) {
			...
			SoDebugError::post((char*)"SoXtClipboard::importSelectionTargets",
					(char*)"- received unknown format %d from clipboard owner", *format );
		}
#endif
	} else {
	...
	if (supportedAtom != 0) {
		XtGetSelectionValue(clipboard->widget, *selectionAtom, supportedAtom,
				(XtSelectionCallbackProc) SoXtClipboard::importSelection,
				(XtPointer) clipboard, clipboard->eventTime);
	}
...
// target, etc.
//
Atom SoXtClipboard::chooseFromImportTargets(Atom *supportedTargets, int length)
//
////////////////////////////////////////////////////////////////////////
		{
	Atom chosenType = 0;
...
// Use: static, private
//
void SoXtClipboard::importSelection(Widget /*w*/, SoXtClipboard *clipboard,
		Atom * /*selectionAtom*/, Atom *target, char *value, uint32_t *length,
		int * /*format*/)
//
////////////////////////////////////////////////////////////////////////
		{
#ifdef DEBUG
	if (clipboard == NULL) {
		SoDebugError::post((char*)"SoXtClipboard::importSelection",
				(char*)"- clipboard is NULL");
		return;
	}
	if (target == NULL) {
	...
	if (clipboard->callbackFunc == NULL)
		return; // no callback to invoke!

	// See if we are interested in the paste target
	const SoXtImportInterest *interest = clipboard->pasteInterest->get(
			clipboard->pasteInterest->find(*target));

	// If there is interest, invoke the paste interest callback to read the data
	if (interest != NULL) {
		(*interest->getCallBack())(interest->getUserData(), *target,
				(void *) value, *length);
	}
...
// Use: protected
//
void SoXtClipboard::pasteImport(Atom dataType, void *data, uint32_t numBytes)
//
////////////////////////////////////////////////////////////////////////
		{
	// Did we get any data?
	...
	if (pathList == NULL) {
		...
		SoDebugError::post((char*)"SoXtClipboard::paste",
				(char*)"could not get Inventor data");
#endif
	}
...
//
SoPathList *
SoXtClipboard::readData(Widget w, Atom target, void *data, uint32_t numBytes)
//
////////////////////////////////////////////////////////////////////////
		{
...
//
void SoXtClipboard::readFile(SoPathList *&pathList, const char *filename)
//
////////////////////////////////////////////////////////////////////////
		{
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::paste", (char*)"could not open file %s", filename);
#endif
...
#ifdef DEBUG
		SoDebugError::post((char*)"SoXtClipboard::paste", (char*)"could not read Inventor data from file %s", filename);
#endif
	}
}
...
// Use: static, private
//
void SoXtClipboard::loseSelection(Widget /*w*/, Atom *selectionAtom)
//
////////////////////////////////////////////////////////////////////////
		{
	// get the clipboard pointer for this object
