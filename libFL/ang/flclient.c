	...
	if (list) {
		for (i = 0, p = fontList; *p; p++, i++) {
			for (p2 = p++; (c = *p) && c != ','; p++)
				;
			*p = '\0';
			list[i] = flGetFontInfo(
					(FLfontNumber) strtoul((char*) p2, NULL, 10));
			*p = c;
		}
		list[i] = NULL;
	}
...
void flAAColor(GLuint colorFull, GLuint colorHalf) {
...
void flAACpack(GLuint colorFull, GLuint colorHalf) {
	FLcontext ctx = current_context;
...
FLcontext flCreateContext(const GLubyte *fontPath, GLint fontNamePreference,
		const GLubyte *fontNameRestriction, GLfloat pointsPerUMx,
		GLfloat pointsPerUMy) {
	...
	TRACE(
			("flCreateContext: fontPath=[%s], ctx=%p\n", SAFE_STR(fontPath), ctx));
	return ctx;
}

FLfontNumber flCreateFont(const GLubyte *fontName, GLfloat mat[2][2],
	...
	TRACE(
			("flCreateFont: fontName=[%s], count=%d\n", SAFE_STR(fontName), charNameCount));
	...
	if (fn > ctx->numFont) {
		...
		if (!(table = (FLfontStruct **) realloc(ctx->fontTable,
				(ctx->numFont + 1) * sizeof(FLfontStruct *)))) {
			ctx->numFont--;
			_flDestroyFont(fs);
			return BAD_FONT_NUMBER;
		}
...
void flDestroyContext(FLcontext ctx) {
	FLfontNumber fn;
...
void flDestroyFont(FLfontNumber fn) {
	TRACE(("flDestroyFont: fn=%" PRIiPTR "\n", fn));
	_flDestroyFont(flGetFontInfo(fn));
}

void flDrawCharacters(GLubyte *str) {
	TRACE(("flDrawCharacters\n"));
	/* TODO - not used in Inventor */
}

void flDrawNCharacters(void *str, GLint charCount, GLint bytesPerCharacter) {
	TRACE(
			("flDrawNCharacters: count=%d, bytesPerChar=%d\n", charCount, bytesPerCharacter));
	/* TODO - not used in Inventor */
}

void flEnumerateFonts(void (*callback)(GLubyte *)) {
	TRACE(("flEnumerateFonts\n"));
	/* TODO - not used in Inventor */
}

void flEnumerateSizes(GLubyte *typeface, void (*callback)(GLfloat)) {
	TRACE(("flEnumerateSizes\n"));
	/* TODO - not used in Inventor */
}

void flFreeBitmap(FLbitmap *bitmapPtr) {
	TRACE(
			("flFreeBitmap: bitmapPtr=%p, bitmapPtr->bitmap=%p\n", bitmapPtr, bitmapPtr ? bitmapPtr->bitmap : NULL));
...
void flFreeScalableBitmap(FLscalableBitmap *bitmapPtr) {
...
void flFreeFontInfo(FLfontStruct *fontStruct) {
	TRACE(("flFreeFontInfo\n"));

	if (fontStruct) {
		if (fontStruct->properties)
			free(fontStruct->properties);
...
void flFreeFontNames(GLubyte **list) {
...
void flFreeFontSizes(GLfloat *list) {
...
void flFreeOutline(FLoutline *outline) {
	TRACE(("flFreeOutline\n"));
...
GLboolean flGetOutlineBBox(FLfontNumber fn, GLuint c, GLfloat * llx,
		GLfloat * lly, GLfloat * urx, GLfloat * ury) {
...
FLcontext flGetCurrentContext(void) {
	TRACE(("flGetCurrentContext: ctx=%p\n", current_context));
	return current_context;
}

FLfontNumber flGetCurrentFont(void) {
	FLcontext ctx = current_context;
...
GLboolean flGetStringWidth(FLfontNumber fn, GLubyte * str, GLfloat * dx,
		GLfloat * dy) {
...
GLboolean flGetStringWidthN(FLfontNumber fn, void * str, GLint charCount,
		GLint bytesPerCharacter, GLfloat * dx, GLfloat * dy) {
...
GLboolean flMakeCurrentContext(FLcontext ctx) {
...
GLboolean flMakeCurrentFont(FLfontNumber fn) {
	FLcontext ctx = current_context;
...
FLfontNumber flScaleRotateFont(const GLubyte *fontName, GLfloat fontScale,
		GLfloat angle) {
...
void flSetHint(GLuint hint, GLfloat hintValue) {
	FLcontext ctx = current_context;
	if (hint > 0 && hint <= 8)
		ctx->hintValue[hint] = hintValue;
}

void flGetFontBBox(FLfontNumber fn, GLfloat * llx, GLfloat * lly, GLfloat * urx,
		GLfloat * ury, GLint * isfixed) {
...
GLboolean flUniGetOutlineBBox(GLubyte *fontList, /* "fn1,fn2,fn3, ..." */
GLubyte *UCS2, GLfloat *llx, GLfloat *lly, GLfloat *urx, GLfloat *ury) {
