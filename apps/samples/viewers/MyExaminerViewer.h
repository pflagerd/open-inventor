...
public:
	MyExaminerViewer(Widget parent = NULL, const char *name = NULL,
			SbBool buildInsideParent = TRUE, MyFullViewer::BuildFlag flag =
					BUILD_ALL, MyViewer::Type type = BROWSER);
...
protected:
	...
	SoEXTENDER
	MyExaminerViewer(Widget parent, const char *name,
			SbBool buildInsideParent, MyFullViewer::BuildFlag flag,
			MyViewer::Type type, SbBool buildNow);
...
private:
	...
	static void feedbackSizeWheelCB(Widget, MyExaminerViewer *p,
			XtPointer *d);
