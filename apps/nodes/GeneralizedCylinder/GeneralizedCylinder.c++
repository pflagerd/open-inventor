...
void GeneralizedCylinder::initClass()
//
////////////////////////////////////////////////////////////////////////
{
...
////////////////////////////////////////////////////////////////////////
{
	...
	SO_KIT_ADD_CATALOG_ENTRY(inputSwitch, SoSwitch, TRUE, topSeparator,
			childList, FALSE);
	SO_KIT_ADD_CATALOG_ENTRY(profileCoords, SoCoordinate3, TRUE, inputSwitch,,
			TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(crossSectionCoords, SoCoordinate3, TRUE,
			inputSwitch,, TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(spineCoords, SoCoordinate3, TRUE, inputSwitch,,
			TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(twistCoords, SoCoordinate3, TRUE, inputSwitch,,
			TRUE);
	...
	SO_KIT_ADD_CATALOG_ENTRY(outputGrp, SoGroup, TRUE, topSeparator, childList,
			FALSE);
	SO_KIT_ADD_CATALOG_ENTRY(shapeHints, SoShapeHints, TRUE, outputGrp,, TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(textureBinding, SoTextureCoordinateBinding, TRUE,
			outputGrp,, TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(textureCoords, SoTextureCoordinate2, TRUE,
			outputGrp,, TRUE);
	...
	SO_KIT_ADD_CATALOG_ENTRY(triangleStripSet, SoTriangleStripSet, TRUE,
			outputGrp,, TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(capTextureBinding, SoTextureCoordinateBinding,
			TRUE, outputGrp,, TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(capTextureCoords, SoTextureCoordinate2, TRUE,
			outputGrp,, TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(topCapCoords, SoCoordinate3, TRUE, outputGrp,,
			TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(topCapFaces, SoIndexedFaceSet, TRUE, outputGrp,,
			TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(bottomCapCoords, SoCoordinate3, TRUE, outputGrp,,
			TRUE);
	SO_KIT_ADD_CATALOG_ENTRY(bottomCapFaces, SoIndexedFaceSet, TRUE, outputGrp,,
			TRUE);
...
SbBool GeneralizedCylinder::setUpConnections(SbBool onOff, SbBool doItAlways) {
...
void GeneralizedCylinder::setDefaultOnNonWritingFields() {
...
void GeneralizedCylinder::loadRow(int rowNum, SbVec3f *newCoords) {
...
void GeneralizedCylinder::loadTextureRow(int rowNum, SbVec2f *newCoords) {
	float uVal, vVal;
	...
	vVal = (fullProfile->point[rowNum][1] - profileMinY) * profileOverHeight;
...
void GeneralizedCylinder::getSpineInfo(float paramDist, SbVec3f &center,
		SbRotation &rot) {
	if (spineLength == 0.0) {
...
SbRotation GeneralizedCylinder::getSpinePointRotation(int ind0, int ind1,
		float paramDist) {
...
void GeneralizedCylinder::initSpineKeyPointRotations() {
	SoCoordinate3 *spinC = SO_GET_PART(this, "spineCoords", SoCoordinate3);
	...
	SbMatrix firstAnswerMat(rotatedX[0], rotatedX[1], rotatedX[2], 0,
			rotatedY[0], rotatedY[1], rotatedY[2], 0, rotatedZ[0], rotatedZ[1],
			rotatedZ[2], 0, 0, 0, 0, 1);
	...
	for (int ind = 1; ind < lastInd; ind++) {
		...
		SbMatrix answerMat(rotatedX[0], rotatedX[1], rotatedX[2], 0,
				rotatedY[0], rotatedY[1], rotatedY[2], 0, rotatedZ[0],
				rotatedZ[1], rotatedZ[2], 0, 0, 0, 0, 1);
...
void GeneralizedCylinder::getTwistInfo(float paramDist, SbRotation &twistRot) {
...
void GeneralizedCylinder::initUpdateInfo() {
	...
	SoCoordinate3 *crosC = SO_GET_PART(this, "crossSectionCoords",
			SoCoordinate3);
	SoCoordinate3 *spinC = SO_GET_PART(this, "spineCoords", SoCoordinate3);
	SoCoordinate3 *twisC = SO_GET_PART(this, "twistCoords", SoCoordinate3);
...
void GeneralizedCylinder::updateSurface() {
	initUpdateInfo();
	...
	if (withSides.getValue() == FALSE) {
		...
		case FACE_SET:
			...
			break;
		case TRIANGLE_STRIP_SET:
			myTStripSet = SO_GET_PART(this, "triangleStripSet",
					SoTriangleStripSet);
			setPart("faceSet", NULL);
			setPart("quadMesh", NULL);
			setPart("nurbsSurfaceGroup", NULL);
	...
	if (withSides.getValue()) {
		...
		if (coords->point.getNum() < numCoordsTotal)
			coords->point.insertSpace(0,
					numCoordsTotal - coords->point.getNum());
		else if (coords->point.getNum() > numCoordsTotal)
			coords->point.deleteValues(0,
					coords->point.getNum() - numCoordsTotal);
	}
	...
	if (withTextureCoords.getValue()
		...
		SoTextureCoordinate2 *cTC = SO_GET_PART(this, "capTextureCoords",
	...
	SoIndexedFaceSet *topCapF = SO_GET_PART(this, "topCapFaces",
			SoIndexedFaceSet);
	SoCoordinate3 *botCapC = SO_GET_PART(this, "bottomCapCoords",
			SoCoordinate3);
	SoIndexedFaceSet *botCapF = SO_GET_PART(this, "bottomCapFaces",
			SoIndexedFaceSet);
	...
	if (withTopCap.getValue() && crossSectionClosed.getValue()
		...
		isSectionClockWise = Triangulator::clockWiseTest((*topC), (*topS), 0,
				topC->getNum());
	...
	if (withBottomCap.getValue() && crossSectionClosed.getValue()
		...
		// affect the normal vector we select.
		SbBool isSectionClockWise;
		isSectionClockWise = Triangulator::clockWiseTest((*botC), (*botS), 0,
				botC->getNum());
	...
	if (withSides.getValue()) {
		...
		if (withTextureCoords.getValue()) {
			...
			if (tCN->point.getNum() < numCoordsTotal)
				...
				tCN->point.deleteValues(0,
						tCN->point.getNum() - numCoordsTotal);
...
GeneralizedCylinder::makeVanillaVersion() {
	...
	SoNode *newKid = getPart("callbackList", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("pickStyle", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("appearance.lightModel", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("appearance.environment", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("appearance.drawStyle", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("appearance.material", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("appearance.complexity", FALSE);
	if (newKid)
		answer->addChild(newKid);

	if (withTextureCoords.getValue()) {
		newKid = getPart("appearance.texture2", FALSE);
		if (newKid)
			answer->addChild(newKid);
	}

	newKid = getPart("appearance.font", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("units", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("transform", FALSE);
	if (newKid)
		answer->addChild(newKid);
	if (withTextureCoords.getValue()) {
		newKid = getPart("texture2Transform", FALSE);
		if (newKid)
			answer->addChild(newKid);
	}

	newKid = getPart("shapeHints", FALSE);
	if (newKid)
		answer->addChild(newKid);

	if (withTextureCoords.getValue()) {
		newKid = getPart("textureBinding", FALSE);
		if (newKid)
			answer->addChild(newKid);

		newKid = getPart("textureCoords", FALSE);
		if (newKid)
			answer->addChild(newKid);

		newKid = getPart("texture2", FALSE);
		if (newKid)
			answer->addChild(newKid);
	}

	newKid = getPart("coords", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("faceSet", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("quadMesh", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("nurbsSurfaceGroup", FALSE);
	if (newKid)
		answer->addChild(newKid);

	newKid = getPart("triangleStripSet", FALSE);
	if (newKid)
		answer->addChild(newKid);
	...
	if (withTextureCoords.getValue()
			&& (withTopCap.getValue() || withBottomCap.getValue())
			&& crossSectionClosed.getValue() && numCols >= 4) {
		newKid = getPart("capTextureBinding", FALSE);
		if (newKid)
			answer->addChild(newKid);

		newKid = getPart("capTextureCoords", FALSE);
		if (newKid)
			answer->addChild(newKid);
	}
	...
	if (withTopCap.getValue() && crossSectionClosed.getValue()
			&& numCols >= 4) {
		newKid = getPart("topCapCoords", FALSE);
		if (newKid)
			answer->addChild(newKid);

		newKid = getPart("topCapFaces", FALSE);
		if (newKid)
			answer->addChild(newKid);
	}
	...
	if (withBottomCap.getValue() && crossSectionClosed.getValue()
			&& numCols >= 4) {
		newKid = getPart("bottomCapCoords", FALSE);
		if (newKid)
			answer->addChild(newKid);

		newKid = getPart("bottomCapFaces", FALSE);
		if (newKid)
			answer->addChild(newKid);
	}

	newKid = getPart("childList", FALSE);
	if (newKid)
		answer->addChild(newKid);
...
void GeneralizedCylinder::changeWithTextureCoords(SbBool newWith) {
	...
	if (newWith) {
		...
		SoTexture2Transform *txXf = (SoTexture2Transform *) getPart(
				"texture2Transform", FALSE);
		if (txXf != NULL) {
...
void GeneralizedCylinder::calculateFullProfile() {
	SoCoordinate3 *profC = SO_GET_PART(this, "profileCoords", SoCoordinate3);
	SoCoordinate3 *spinC = SO_GET_PART(this, "spineCoords", SoCoordinate3);
	SoCoordinate3 *twisC = SO_GET_PART(this, "twistCoords", SoCoordinate3);
...
void GeneralizedCylinder::calculateFullCrossSection() {
	SoCoordinate3 *crosC = SO_GET_PART(this, "crossSectionCoords",
			SoCoordinate3);
...
void GeneralizedCylinder::fieldsChangedCB(void *data, SoSensor *sens) {
...
void GeneralizedCylinder::inputChangedCB(void *data, SoSensor *sens) {
	GeneralizedCylinder *s = (GeneralizedCylinder *) data;
...
void GeneralizedCylinder::updateSurroundingManip() {
	SoNode *n;
	...
	SoSurroundScale *ss = (SoSurroundScale *) d->getPart("surroundScale",
	FALSE);
...
void GeneralizedCylinder::changeTransformType(SoType newType) {
	...
	if (newType.isBad()
		...
		SoFullPath *xfPath = (SoFullPath *) createPathToPart("transform",
		FALSE);
		if (!xfPath)
			return;
