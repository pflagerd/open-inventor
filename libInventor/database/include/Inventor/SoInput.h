...
SoINTERNAL struct SoInputFile {
	...
	const void *buffer;	// Buffer to read from (or NULL)
...
public:
	...
	// TRUE (the default).
	void addReference(const SbName &name, SoBase *base, SbBool addToGlobalDict =
			TRUE);
...
private:
	...
	FILE * findFile(const char *fileName, SbString &fullName) const;
